...
exports.constructYoutubeUrl = void 0;
 ...
 */
function constructYoutubeUrl(paramIdentifier, fields, paramId, limit) {
    switch (paramIdentifier) {
        case 'channels':
            // Constructs the URL for fetching channel details of the authenticated user.
            return `${constants_1.Constants.GOOGLE.GOOGLE_YOUTUBE_ENDPOINT}/${paramIdentifier}?part=${fields}&mine=true`;
        case 'search':
            // Constructs the URL for searching videos on a channel.
            if (limit && limit > 0) {
                return `${constants_1.Constants.GOOGLE.GOOGLE_YOUTUBE_ENDPOINT}/${paramIdentifier}?part=${fields}&channelId=${paramId}&type=video&maxResults=${limit}`;
            }
            else {
                return `${constants_1.Constants.GOOGLE.GOOGLE_YOUTUBE_ENDPOINT}/${paramIdentifier}?part=${fields}&channelId=${paramId}&type=video`;
            }
        case 'videos':
            // Constructs the URL for fetching video details.
            return `${constants_1.Constants.GOOGLE.GOOGLE_YOUTUBE_ENDPOINT}/${paramIdentifier}?part=${fields}&id=${paramId}`;
        case 'activities':
        case 'channelSections':
            // Constructs the URL for fetching activities or channel sections.
            if (limit && limit > 0) {
                return `${constants_1.Constants.GOOGLE.GOOGLE_YOUTUBE_ENDPOINT}/${paramIdentifier}?part=${fields}&channelId=${paramId}&maxResults=${limit}`;
            }
            else {
                return `${constants_1.Constants.GOOGLE.GOOGLE_YOUTUBE_ENDPOINT}/${paramIdentifier}?part=${fields}&channelId=${paramId}`;
            }
        case 'captions':
            // Constructs the URL for fetching video captions.
            return `${constants_1.Constants.GOOGLE.GOOGLE_YOUTUBE_ENDPOINT}/${paramIdentifier}?part=${fields}&videoId=${paramId}`;
        case 'i18nLanguages':
        case 'i18nRegions':
            // Constructs the URL for fetching languages or regions.
            return `${constants_1.Constants.GOOGLE.GOOGLE_YOUTUBE_ENDPOINT}/${paramIdentifier}?part=${fields}`;
        case 'playlists':
            // Constructs the URL for fetching playlists.
            return `${constants_1.Constants.GOOGLE.GOOGLE_YOUTUBE_ENDPOINT}/${paramIdentifier}?part=${fields}&channelId=${paramId}`;
        case 'playlistItems':
            // Constructs the URL for fetching playlist items.
            return `${constants_1.Constants.GOOGLE.GOOGLE_YOUTUBE_ENDPOINT}/${paramIdentifier}?part=${fields}&playlistId=${paramId}&maxResult=${limit}`;
        case 'subscriptions':
...
exports.constructYoutubeUrl = constructYoutubeUrl;
